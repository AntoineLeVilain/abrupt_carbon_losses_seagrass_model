...
multisolve <- function(model, lower_limit, upper_limit, iter) {
  ...
  combinations <- expand.grid(lower_upper_list)
  colnames(combinations) <- names(model(0))
  combinations <- rbind(rep(0.1, length(lower_limit)), combinations) # Add a row of zeros
...
# Bifurcation diagram for each scenario
...
for (par2 in c("ma", "pmax", "temp")){
  ...
  par1_ranges <- list(
    ma = seq(14.2, 2000, length.out = 150),
    pmax = seq(14.2, 11000, length.out = 150),
    temp = seq(14.2, 5000, length.out = 150)
  )
  ...
  foreach(par1_index = par1_range, .packages=c("rootSolve", "dplyr", "rlang"))%dopar%{
    ...
    assign(paste("liste", par1, sep = "_"), c()) # sh values
    assign(paste("liste", par2, sep = "_"), c()) # Stress values
    liste_stability <- c() # State of the ecological dynamics at each point

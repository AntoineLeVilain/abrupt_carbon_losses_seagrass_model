...
for (par1 in c("ma", "pmax", "temp")){
  ...
  par1_ranges <- list(
    ma = seq(0, 0.002, length.out = 150),
    pmax = seq(0.5, 2, length.out = 150),
    temp = seq(298.85, 304, length.out = 150)
  )
  
  par1_range <- par1_ranges[[par1]]
  
  par2_range <- c(1000000)
  par2 <- "sh"
  ...
  foreach(par1_index = par1_range, .packages=c("rootSolve", "dplyr", "rlang"))%dopar%{
    ...
    for (par2_index in par2_range){
      ...
      # we determine the system state from the number of equilibrium values
      ...
      if (dim(equilibrium)[1] == 3){ # bistable
        ...
        medium_save <- equilibrium[which(equilibrium[, 1] != max(equilibrium[, 1]) & equilibrium[, 1] != min(equilibrium[, 1])), ]
        ...
        for (index2 in 1:length(model(0))){
          ...
          if (dim(equilibrium)[2] > 1){
            ...
            assign(paste("liste", names(model(0))[index2], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index2], "seagrass", sep = "_")), max_save[1, index2]))
            assign(paste("liste", names(model(0))[index2], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index2], "bistable", sep = "_")), medium_save[1, index2]))
            assign(paste("liste", names(model(0))[index2], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index2], "bare", sep = "_")), min_save[1, index2]))
            
          } else {
            
            assign(paste("liste", names(model(0))[index2], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index2], "seagrass", sep = "_")), max_save[1]))
            assign(paste("liste", names(model(0))[index2], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index2], "bistable", sep = "_")), medium_save[1]))
            assign(paste("liste", names(model(0))[index2], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index2], "bare", sep = "_")), min_save[1]))
        ...
        for (index3 in 1:length(model(0))){
          ...
          if (dim(equilibrium)[2] > 1){
            ...
            assign(paste("liste", names(model(0))[index3], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index3], "seagrass", sep = "_")), max_save[1, index3]))
            assign(paste("liste", names(model(0))[index3], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index3], "bistable", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index3], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index3], "bare", sep = "_")), min_save[1, index3]))
            
          } else {
            
            assign(paste("liste", names(model(0))[index3], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index3], "seagrass", sep = "_")), max_save[1]))
            assign(paste("liste", names(model(0))[index3], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index3], "bistable", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index3], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index3], "bare", sep = "_")), min_save[1]))
        ...
        for (index4 in 1:length(model(0))){
          ...
          if (dim(equilibrium)[2] > 1){
            ...
            assign(paste("liste", names(model(0))[index4], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index4], "seagrass", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index4], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index4], "bistable", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index4], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index4], "bare", sep = "_")), equilibrium[1, index4]))
            
          } else {
            
            assign(paste("liste", names(model(0))[index4], "seagrass", sep = "_"), c(get(paste("liste", names(model(0))[index4], "seagrass", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index4], "bistable", sep = "_"), c(get(paste("liste", names(model(0))[index4], "bistable", sep = "_")), NA))
            assign(paste("liste", names(model(0))[index4], "bare", sep = "_"), c(get(paste("liste", names(model(0))[index4], "bare", sep = "_")), equilibrium[1, 1]))
    ...
    for (index5 in 1:length(model(0))){
      ...
      data <- cbind(data, get(paste("liste", names(model(0))[index5], "seagrass", sep = "_")))
      data <- cbind(data, get(paste("liste", names(model(0))[index5], "bistable", sep = "_")))
      data <- cbind(data, get(paste("liste", names(model(0))[index5], "bare", sep = "_")))
      
      list_names <- c(list_names, paste(names(model(0)[index5]), "seagrass", sep = "_"))
      list_names <- c(list_names, paste(names(model(0)[index5]), "bistable", sep = "_"))
      list_names <- c(list_names, paste(names(model(0)[index5]), "bare", sep = "_"))

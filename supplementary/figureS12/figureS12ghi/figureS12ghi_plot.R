...
setwd('/supplementary/figureS12/figureS12ghi') # set your own working directory
...
data <- read.csv("hea_temp.csv")

data2 <- read.csv("hea_ma.csv")

data3 <- read.csv("hea_pmax.csv")

# plot1

plot_1 <- ggplot() +
  geom_point(data = data, aes(x = temp, y = hea, color = stability), size = 1, shape = 15) +
...
plot_2 <- ggplot() + geom_point(data = data2, aes(x = ma, y = hea, color = stability), size = 1, shape = 15) + 
  scale_color_manual(breaks = c("seagrass", "bistable", "bare"),
                     values=c("#52854C", "black","#C4961A")) +
...
plot_3 <- ggplot() + geom_point(data = data3, aes(x = pmax, y = hea, color = stability), size = 1, shape = 15) + 
  scale_color_manual(breaks = c("seagrass", "bistable", "bare"),
                     values=c("#52854C", "black","#C4961A")) +
  xlab("Pmax") + ylab("Sh") + theme_classic() + theme(legend.position = "none") + xlim(0.5, 2.5)

# Save plots as a vectorized PDF
tiff("hea.tiff", units = 'in', width = 15, height = 3, res = 700)
